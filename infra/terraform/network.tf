...
resource "aws_security_group" "backend" {
  ...
  description = "Backend tasks; ingress from the load balancer only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "analytics" {
  name        = "${local.name}-analytics-sg"
  description = "Analytics tasks; ingress from the backend only, never the internet"
  vpc_id      = aws_vpc.main.id

  ingress {
    description     = "Analytics port from the backend"
    from_port       = 8000
    to_port         = 8000
    protocol        = "tcp"
  ...
  tags = { Name = "${local.name}-analytics-sg" }
}

resource "aws_security_group" "database" {
  name        = "${local.name}-db-sg"
  description = "RDS; ingress from the backend tasks only"
  vpc_id      = aws_vpc.main.id

  ingress {
    description     = "MySQL from the backend"
    from_port       = 3306
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [aws_security_group.backend.id]
